...
# i want to import paginator
from django.core.paginator import Paginator



from classroom.models import *
from blog.models import table_model 
# Category, Grade, Batch_Category
...
def Categories(request,id):
	categories = Mem.objects.filter(id=id)
...
def CategoryCourses(request, category_slug):
	mem = get_object_or_404(Mem, slug=category_slug)
	courses = Course.objects.filter(mem=mem)
	context = {
		'mem': mem,
		'courses': courses,
	}
...
@login_required
def CourseDetail2(request, course_id):
	user = request.user
	course = get_object_or_404(Course, id=course_id)
	teacher_mode = False
	...
	return render(request, 'classroom/course2.html', context)
...
def savedpq(request):
	...
	return render(request, 'temp/savepq.html', context)
