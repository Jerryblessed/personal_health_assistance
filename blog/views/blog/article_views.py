...
from blog.models.course_model import Article, Course

class ArticleCoursesListView(ListView):
    model = Course
    paginate_by = 12
    context_object_name = 'courses'
    template_name = 'blog/article/article_courses.html'

    def get_queryset(self):
        article = get_object_or_404(Article, slug=self.kwargs.get('slug'))
        return Course.objects.filter(article=article, status=Course.PUBLISHED, deleted=False)

    def get_context_data(self, **kwargs):
        context = super(ArticleCoursesListView, self).get_context_data(**kwargs)
        article = get_object_or_404(Article, slug=self.kwargs.get('slug'))
